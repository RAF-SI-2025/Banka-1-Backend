...
This service is currently a scaffold, meaning it is the initial foundation for further stock functionality.
...
- stock business endpoints and stock tables have not been implemented yet
...
At the moment there is only a bootstrap migration:

- `src/main/resources/db/changelog/001-baseline.sql`

That is intentional, because stock domain tables have not been defined yet.
...
5. registers the JWT decoder and security filter chain from `security-lib`
6. registers the `RestClient` bean for `exchange-service`
7. exposes the actuator health endpoints
...
- there are currently no test classes implemented
- the Gradle task still confirms that the module compiles and the configuration is valid
